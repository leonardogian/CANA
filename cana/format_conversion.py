# source: https://github.com/rionbr/CANA/pull/5/files
import os, re
...
def _parse_bool_func(exp, NodeDict, NodeList_sorted=None):
    ...
    # avoid single letter node name like "L", which would collide with 'NodeValueList'
    sublist = [(varlist[i], '*%~' + str(NodeDict[varlist[i]] - 1) + '~%*') for i in range(len(varlist))]
    ...
    # step 2 substitution
    newexp = newexp.replace('*%~', 'NodeValueList[')
...
def CC_bool_expr_folder_to_cnet(expfolder, outputfile):
    ...
    '''
    CC_bool_expr2cnet(os.path.join(expfolder, 'expressions.ALL.txt'),
                      os.path.join(expfolder, 'external_components.ALL.txt'), outputfile)
